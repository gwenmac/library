<template>
  <div class="field">
    ...
    <div class="picker-row">
      ...
      <input v-else v-model="newName" :placeholder="newPlaceholder" />
    ...
    async add() {
      if (this.creatingNew) {
        ...
        try {
          ...
          if (!res.ok) {
            this.$emit('error', 'Failed to create ' + this.label.toLowerCase() + ' (API returned ' + res.status + ')')
            return
          }
          ...
          this.$emit('error', 'Failed to create ' + this.label.toLowerCase() + ': ' + err.message)
        }
      } else {
